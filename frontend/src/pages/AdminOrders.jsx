...
export default function AdminOrders() {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    if (userFilter !== 'all' && order.user_email !== userFilter) return false;
    return true;
  });

  const materials = [...new Set(orders.map(o => o.material_name))];
  const users = [...new Set(orders.map(o => o.user_email))];
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={{ ...styles.title, color: colors.text }}>All Orders</h2>
        <p style={{ ...styles.subtitle, color: colors.textSecondary }}>Manage and monitor the print queue</p>
      </div>
      ...
      {orders.length > 0 && (
        <div style={{ ...styles.filters, background: colors.filterBg, borderColor: colors.cardBorder }}>
          ...
          {(statusFilter !== 'all' || materialFilter !== 'all' || userFilter !== 'all') && (
            <button
              onClick={() => { setStatusFilter('all'); setMaterialFilter('all'); setUserFilter('all'); }}
      ...
      {!loading && orders.length === 0 && (
        <div style={{ ...styles.empty, background: colors.card, borderColor: colors.cardBorder }}>
          ...
          <p style={{ color: colors.textSecondary }}>Orders will appear here once customers place them.</p>
        </div>
      )}
      ...
      {filteredOrders.map((o) => <OrderCard key={o.id} order={o} admin onStatusChange={handleStatusChange} />)}
    </div>
  );
}
